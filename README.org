...
- Building them using ~pkgs.buildNpmPackage~, using the upstream repositories as the ~src~, would be ideal, but their build scripts include ~spago~ invocations that will fail in the Nix build sandbox (see [[https://github.com/ngua/spago.nix][spago.nix]] for a more detailed discussion of why building Spago project with Nix is very difficult, at least in its current incarnation).
